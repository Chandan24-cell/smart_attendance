...
from backend.models import AttendanceRecord, AttendanceSession, Setting, Student, db
...
def send_30day_report():
    ...
    try:
        ...
        return False, "❌ Login failed! Gmail requires an App Password."
...
def check_auto_report():
    """Send the report once every 30 days when the engine runs."""
    today = datetime.now()
    ...
    if ok:
        ...
        if s:
            ...
            db.session.add(Setting(setting_key="last_report_date", setting_value=today.strftime("%Y-%m-%d")))
        db.session.commit()
